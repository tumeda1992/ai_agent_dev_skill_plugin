...
**提案を出す前に確かめる。** 中心の問いは「いま出している層の一つ上は埋まっているか」である。提案は why / what / how の設計 / how の四層の上に乗る。why を主語にすると、why が決まっていることだけを確認して通過できるため、主語を「いま出している層」へ置く。この問いが抽象で発火しないときは次の三つを当てる。
...
この phase の実施中に観測した四つの case を、確定した段へ当てた。

**発火する二件。**

1. **「`facilitate-discussion` から `think_standards` への参照は無い」と結論した。** 検索対象を `think_standards` に絞ったため、`documentation_standards/confidentiality.md` への既存参照を見落とした。主軸「この提案は、why からこのレイヤまで引いて説明できるか」で引けない。「前例が無い」という前提が確定しておらず、補助「この提案が依拠する前提は確定しているか」が発火する。
2. **旧 file への残存参照を `plugins` / `docs` / `.agents` で調べ、`scripts` を外した。** その結果 `validate-plugin.mjs` の必須 path 登録を見落とし、validation が失敗した。同じ補助が発火する。検索範囲を絞ったこと自体は誤りでないが、絞った範囲で「なし」と結論したことが前提の未確定にあたる。

**発火しない二件。**

3. **commit 計画を立てずに commit した。** 提案を出す場面ではない。材料は揃っており、手順を飛ばしたことが原因である。この段は発火しないし、発火すべきでもない。
4. **論点4 の提案0 へ二つの decision を入れた。** 提案を出す場面ではあるが、材料は揃っていた。分割の規律は `facilitate-discussion` の不変条件として既知であり、適用を漏らしたことが原因である。この段ではなく既存 gate の問題にあたる。

四件とも、発火するかしないかを判定できた。発火する二件はいずれも「調べれば分かることを調べずに前提として置いた」形であり、補助の一つ目が捉える。発火しない二件は材料不足ではなく規律の適用漏れであり、この段の対象外として区別できる。
