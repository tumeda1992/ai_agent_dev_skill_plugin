...
このpluginは、利用先の作業で表面化した四つの欠落をまだ持っている。designの受け入れ基準が「動くこと」だけで運用の引き継ぎを見ていないこと、実環境の挙動に依存する論点を机上で決めさせ続けること、hookの注入文面がskill未起動を正常に見せること、そしてこのrepositoryが自身を文脈なく「正本」と名乗ることである。この四つを、それぞれ規範・進行契約・注入文面・用語として直す。終了時には、infrastructure構築を含むsteeringが運用の引き継ぎを受け入れ基準として持ち、実環境依存の論点を基盤構築後へ回す判断基準が進行契約に載り、hookの注入が自身を要約と名乗り、このrepositoryが自身を相対語で呼ばない状態になる。
...
`task-design` は「設計を尽くしてから実装する」ことを目的にしているが、実行しなければ確定しない事実がある。この skill は既にそれを認め、技術検証実装を `spike/` へ置いて「書いたコードは捨ててよい。残るのは事実だけ」と定義している。基盤構築はこの定義から外れる。基盤は作れば残るため、捨てられない実行によってしか得られない事実が存在する。
...
実環境の挙動に依存し、かつ作るものそのものを変えない論点は、その段階の design scope へ含めない。基盤を立て、実測を得てから次の段階で設計する。判定は二問で行い、両方が Yes の場合は基盤より前に決める。作るものが変わる論点を後回しにすると、基盤そのものが不要になりうるためである。

1. その論点の答えは、実環境の挙動に依存するか
...
1. `task-design` は不確実性ごとに、その答えが実環境の挙動に依存するか、作るものそのものを変えるかを判定する。実環境依存かつ作るものを変えないものは、その段階の scope から外し、`design.md` の非目標へ「基盤の実測後に設計する」と記す
...
| --- | --- | --- |
| `plugins/tumeda-dev/skills/task-design/SKILL.md` | 設計を進めるagent | 実環境依存の不確実性を今回の段階の scope へ含めるかを判定できる |
...
- section 3 へ `### 3-5. 実環境依存の論点は、基盤を立ててから設計する` を挿入する。位置は `3-4 不確実性のためならコードを書く` の直後。既存の `3-5 設計は対話であり転記ではない` は `3-6` へ繰り下がる
- 挿入する節は section 3 の型（`**思想:**` / `**違反シグナル:**` / `**帰結:**` / `**問い:**`）に従う。思想として書くのは「設計を尽くした単位は実行可能な一段階である」「実環境の挙動に依存し、かつ作るものそのものを変えない論点はその段階の scope へ含めない」「二問で判定し、両方 Yes なら基盤より前に決める」「途中の design.md は最終形を示さないため対象範囲を明示する」
...
- `task-design/SKILL.md` に `3-5` として実環境依存の論点を扱う節があり、既存節の繰り下げと節番号の参照更新が済んでいる
