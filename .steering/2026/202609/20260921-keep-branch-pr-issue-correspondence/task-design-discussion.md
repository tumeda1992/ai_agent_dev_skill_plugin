...
## 論点7: `create_or_get_pr.sh` が既存形式をどこまで受けるか

**ステータス:** 決定

**種別:** TBDヒアリング

### イテレーション0: 後方互換の範囲を決める
...
declares_feature_issue_contract() {
  # context file の `### Branch / issue 契約` section を読み、
  # `feature-<issue番号>` という文字列が含まれるか判定する
}

if declares_feature_issue_contract && [[ "$head" =~ ^feature-([0-9]+)$ ]]; then
  issue_id="${BASH_REMATCH[1]}"
  title="${title:-$(issue_title "$issue_id")}"
  body="Closes #${issue_id}"          # body 未指定なら自動で付く
fi
```

**`Closes` が自動で付く。** 検証のための merge で `--body` を指定し忘れると、`Closes` が付いた PR ができる。この phase の起点となった事故の一因である。

##### 変更後に満たすこと（論点1・5 の decision から一意に決まる部分）

```text
本線（issue-<番号>）        Closes #<番号> を自動で付けてよい
派生（issue-<番号>-<説明>）  Closes を付けない。参照として書く
形式の宣言                   context file の `### Branch / issue 契約` が持つ
```

##### 未決 — 既存形式をどこまで受けるか

利用先 repository は現在 `feature-<issue番号>` を宣言している。

##### a. 両形式を受ける

`feature-<番号>` と `issue-<番号>[-<説明>]` の両方を判定する。`feature-` 形式には派生の概念が無いため、本線として扱う。

##### b. 新形式のみ受ける

既存の利用先は context file の宣言を `issue-<番号>` 形式へ変える。宣言を変えるまでは、script は自動 title/body を付けず、引数に委ねる。

##### c. 形式を script から外し、context file に正規表現を書かせる

**推す案は b。ただし a との差は小さい。**

#### 提案背景

**この論点が扱う範囲。** 論点5 で「標準は原則と推奨形式を定め、利用先が context file で宣言する」と決めた。script がその宣言をどう読むかは決めたが、既存形式をどこまで受けるかは決めていない。実装中に判明した。

**b を推す理由。**

利用先は現在一つであり、context file の宣言を一行変えれば移行できる。`feature-` 形式を残すと、script が二つの形式を判定し続けることになる。`feature-` 形式には派生の概念が無いため、検証用 branch を作るときは結局 `issue-` 形式が要る。二形式が並存すると、どちらを使うかの判断が利用者に残る。

宣言を変えるまでの間も、script は停止せず動く。自動 title/body が付かないだけであり、`--title` と `--body` を渡せば従来どおり PR を作れる。

**a を採らない理由。** 二形式の判定を script が持ち続ける。移行が終わっても、判定を外す契機が無い。

**c を採らない理由。** context file へ正規表現を書かせると、宣言が実装詳細になる。現在の宣言は自然言語で書かれており、人が読む前提である。script のために形式言語を持ち込むと、context file の性質が変わる。

**b を採った場合に必要になること。** 利用先 repository の context file の宣言を `issue-<番号>` 形式へ変える作業は、この phase の scope 外である（この repository の変更ではない）。標準と script を先に整え、利用先での移行は利用先の作業として残す。

#### 提案0へのフィードバック

**結果:** a を採用。提案0 は二つの decision を含んでいた。

> aとbの差は何？ そして、派生ではClosesを付けないことは金科玉条？ 検証したissueを追っても別にいいと思うけど

> a

**a と b の差は一点だけである。** 移行期間中に自動 title/body が効くかどうかであり、script が判定を二つ持つか一つ持つかの違いに還元される。提案0 は「差は小さい」と書きながら b を推しており、推す根拠が弱かった。

**提案0 は二つの decision を含んでいた。** 形式の受け入れ範囲と、body のデフォルトである。独立して却下できるため、後者を論点8 へ分ける。

### 決定

`create_or_get_pr.sh` は `feature-<番号>` と `issue-<番号>[-<説明>]` の両形式を受ける。context file の `### Branch / issue 契約` section が宣言した形式を判定に使う。

`feature-` 形式には派生の概念が無いため、常に本線として扱う。

---

## 論点8: PR body のデフォルトをどうするか
...
##### 訂正 — 派生形式と `Closes` の有無は独立している

論点7 の提案0 は「本線は `Closes`、派生は参照」を実装案として書いた。これは論点1 の decision と矛盾する。

```text
論点1 の decision   経路の選択は「この merge で issue の作業が終わるか」で判定する
```

判定は作業が終わるかであり、branch 名ではない。

```text
検証用 branch（issue-42-verify-ci）      作業は終わらない  → Closes なし
作業を分割した branch（issue-42-part2）   それで終わるなら  → Closes あり
```

**派生 branch であることは、`Closes` を付けない理由にならない。** 禁じているのは検証のための merge へ付けることだけである。issue への参照そのものは、検証用 PR でも付けたほうがよい。

##### a. 本線は `Closes`、派生は参照をデフォルトにする

##### b. 常に参照をデフォルトにする。`Closes` は `--body` で明示的に渡す

```bash
# 本線でも派生でも
body="参照: #${issue_id}"

# issue を閉じる PR では明示的に渡す
create_or_get_pr.sh --body "Closes #42"
```

##### c. body を自動生成しない。常に `--body` を要求する

**推す案は b。**

#### 提案背景

**この論点が扱う範囲。** 論点7 で両形式を受けることが決まった。script が body をどう生成するかを決める。

**b を推す理由。**

`Closes` の有無は「この merge で issue の作業が終わるか」で決まり、branch 名からは判定できない。a は branch 名で決めるため、派生 branch で issue が終わる場合に誤ったデフォルトを出す。

加えて、この phase の起点となった事故は `Closes` が自動で付いたことが一因である。`--body` を指定し忘れると `Closes` が付く構造であった。`Closes` は issue を閉じる操作であり、明示的な行為にするのが安全側である。

参照は常に付けてよい。branch 名から issue 番号を引けるため自動生成でき、付けて困る場面が無い。

**a を採らない理由。** branch 名で `Closes` の有無を決めると、決定の判定基準（作業が終わるか）と実装の判定基準（branch 名）が食い違う。食い違ったまま運用すると、派生 branch で issue を閉じたい場面で毎回 `--body` を渡すことになり、その手間が「本線でやればいい」という判断を誘う。

**c を採らない理由。** 参照の自動生成まで失う。issue 番号は branch 名から一意に引けるため、自動化して困らない。

**既存の挙動が変わることを明示する。** 現在 `feature-<番号>` 形式では `Closes #<番号>` が自動で付く。b を採ると参照に変わる。既存の利用先で本線の PR を作る際、`--body` を渡さなければ `Closes` が付かなくなる。これは事故の原因を取り除く変更であり、意図した破壊である。
...
`create_or_get_pr.sh` は body のデフォルトを `参照: #<番号>` とする。`Closes` は `--body` で明示的に渡す。

`Closes` の有無は「この merge で issue の作業が終わるか」で決まり、branch 名からは判定できない。加えて、この phase の起点となった事故は `Closes` が自動で付いたことが一因である。issue を閉じる操作は明示的な行為にする。

既存の `feature-<番号>` 形式でも参照に変わる。事故の原因を取り除く、意図した挙動変更である。

**派生形式であることは `Closes` を付けない理由にならない。** 禁じているのは検証のための merge へ付けることだけである。この区別を標準へ明示する。

---

## 論点9: 派生 branch の PR title をどう決めるか

**ステータス:** 決定

**種別:** TBDヒアリング

### イテレーション0: title の生成規則を決める
...
```bash
if declares_feature_issue_contract && [[ "$head" =~ ^feature-([0-9]+)$ ]]; then
  title="${title:-$(issue_title "$issue_id")}"      # issue のタイトルを使う
else
  title="${title:-$(default_title_from_branch "$head")}"   # branch 名から生成
fi
```

`feature-` 形式には派生が無いため、二分岐で足りていた。`issue-<番号>-<説明>` を受けると、派生が「issue 番号を引ける」側に入る。

##### a. 本線は issue title、派生は branch 名から生成する

```text
issue-42             → issue #42 のタイトル
issue-42-verify-ci   → "issue 42 verify ci"（branch 名から生成）
```

##### b. どちらも issue title を使う

##### c. どちらも branch 名から生成する

**推す案は a。**

#### 提案背景

**この論点が扱う範囲。** 論点8 で body のデフォルトが決まった。title の生成規則は別であり、実装中に判明した。

**a を推す理由。** 派生 branch の PR に issue のタイトルを付けると、本線の PR と見分けがつかない。PR 一覧で同じタイトルが並び、どちらが issue を完了させる PR かを開かないと判定できない。branch 名から生成すれば、`verify-ci` のような差分が title に現れる。

**b を採らない理由。** 上記のとおり見分けがつかなくなる。

**c を採らない理由。** 本線の PR で issue のタイトルを使えなくなる。本線は issue の作業そのものであり、title を二重に考える必要が無い。
...
本線は issue のタイトルを使い、派生は branch 名から生成する。

派生 branch の PR に issue のタイトルを付けると本線の PR と見分けがつかない。PR 一覧で同じタイトルが並び、どちらが issue を完了させる PR かを開かないと判定できなくなる。
