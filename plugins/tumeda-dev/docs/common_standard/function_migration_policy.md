...
### 文字列assertionに衝突したら、契約が変わったのかを先に問う

検査scriptがピン留めした文字列と衝突したとき、assertionを新しい文言へ書き換える前に問う。

- 判断の問い: **「変えようとしているのは契約か、それとも曖昧さ・言い回しだけか」**
- 契約が変わった → ピン留め側を新しい契約へ更新する
- 契約は変わっていない → ピン留めされた文を保存し、直後に補う文を足す。ピン留めを外さない

やってしまいがちな失敗: 検査が落ちたことを「検査側が古い」と解釈し、assertionを新文言へ書き換えて通す
→ 契約が変わっていないのにguardだけが緩む。次に同じ文が消えても検知できなくなる
→ 正しい問い: 「この検査は何を守るために置かれたか。いま自分はそれを壊していないか」

適用例: ある契約文の指示対象が曖昧だったため言い換えようとしたところ、検査scriptのピン留めに衝突した。契約そのもの（受け取った時に何をやり直すか）は変わっていなかったため、ピン留め文を保存したまま、直後へ指示対象を明示する一文を足した。検査scriptは変更していない。
