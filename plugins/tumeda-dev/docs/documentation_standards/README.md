...
- **[confidentiality.md](./confidentiality.md)** — その記述が公開されたとき読んだ人が何をできるようになるかで、document へ書いてよいかを判定する標準（機密性）。他の標準と判定の向きが逆で、読者が使えるかではなく、書いてはいけないものが入っていないかを見る。
...
- 観点には向きが 2 つある。読者がその doc で使えるかを見るものと、書いてはいけないものが入っていないかを見るものである。向きが違うと適用範囲が違う。前者は読者向けの doc にだけ当たり、steering の作業記録には当たらない。後者は公開範囲へ置かれるものすべてに当たるため、`document-review` へ足すだけでは届かず、作業記録を書く skill 側にも trigger が要る。標準を増やすとき、どちらの向きかを判断してから参照側を決める。
  - やってしまいがちな失敗: 「書いてはいけないものが入っていないか」を見る標準を作り、`document-review` の観点 list へ足して完了とする。`document-review` の対象は docs と skill 本体であり、steering の作業記録には当たらないため、作業記録へ書く経路が塞がらない。
