...
- **MUST: 種別を記載する**（**TBDヒアリング** / **認識齟齬** / **レビュー指摘**。複数該当時は併記可）
  - TBDヒアリング: ユーザーの判断・好み・方針確認で決まる論点。振り返りで再発防止規則へ自動昇格させない
  - 認識齟齬: AIとユーザーの前提・対象範囲・用語のずれから生まれた論点。共有されていなかった前提を振り返る
  - レビュー指摘: design.md / tasklist.md等を見た後に判明した漏れ・補完。次回の設計観点として先取りできるか振り返る
  - 種別は検索・振り返りの軸であり、事象・原因・提案・検証の記述を省略するためには使わない
