...
- **MUST**: 対象actionがある場合は、引き継ぎの観点（この作業を破棄しても残る変化を、後から更新・確認・復旧できること）を`design.md`の受け入れ基準へ入れる。停止・確認taskでは、設計時点に決めた運用documentの記述と実態が合っているかを照合する。実装の過程で方針が変わっていた場合は、その場でdocumentを書き換えず`design.md`へ戻す。
...
- 未開拓の実行基盤を伴う作業では、基盤を立てるphaseを最初に置き、そのphaseのDoDへ実測結果を含める。完了後は実測結果を入力として`design.md`へ戻る前提でplanを作る。基盤phaseの外側にある後続phaseは、実測が得られるまで詳細を確定できないため、基盤phase完了後に改めて設計する。
...
- [ ] **`design.md`が定める変更対象を、tasklistのtaskが覆っているか**: `design.md`の付録「変更の実行区分」と完成後の姿が挙げる対象を一つずつ辿り、対応するtaskがあることを確認する。
  - design側から辿る。taskから逆に辿らない。taskから辿ると、存在するtaskはすべて何かを実装しているため必ず整合して見える。欠落は「design側に対応taskが無い対象がある」という形でしか現れない。
  - やってしまいがちな失敗: discussionで中心的に扱った対象だけをphaseへ落とす。中心でなかった対象が静かに落ち、tasklistだけを見ても欠落に気づけない。
  - 前項の「DoDの各項目に、それを担保するtaskがあるか」はtasklist内部の整合を見る。この項目はdesignとtasklistの間の覆域を見る。片方が通っても他方は通らない。
