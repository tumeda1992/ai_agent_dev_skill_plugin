...
WHY（なぜ作るか）→ WHAT（何を作るか）→ HOW（どう作るか）は、**合意の順序**である。記述の順序ではない。WHAT を間に挟むのは「目的に対するスコープ」を独立した合意点にするためである。

ただし、すべての変更で WHY の合意が要るわけではない。要るのは、**上位が下位を規定する場合**である。

「`path/to/dir/hoge.ts` のこの文言をこう変えて」という依頼に WHY は書かれていない。類推もできない。古いのか、typo なのか、表現を統一したいのか、依頼文からは判別できない。それでも問題が起きないのは、類推する必要がないからである。書き換える内容は WHY が何であれ変わらない。上位は下位を規定していない。

規定する場合は違う。WHY が変われば何を作るかが変わり、何を作るかが変われば構成が変わる。裏を返せば、WHY が合意されていれば HOW はそこから導かれる。だからこそ、WHY の合意なしに下位を書くと、WHY が動いた時点で下位が崩れる。崩れるのは記述だけでなく、下位を導いた基準そのものである。

判定は二段で行う。

1. いま書こうとしている部分は、上位が変わったら変わるか。**上位を別の答えに置き換えてみて、下位が変わるかを見る。** 変わらないなら上位は要らない。そのまま書いてよい
2. 変わるなら、その上位が共有されているかを確認する。相手の言葉を引用できないなら、書かずに `TBD:` として残す

どのレイヤが依頼に書かれているかで判定しない。HOW が指示されている依頼も、WHAT だけの依頼もある。書かれているかではなく、いま書こうとしているものが上位によって変わるかを見る。

**違反シグナル:**
- 上位を別の答えに置き換えると下位が変わるにもかかわらず、その上位について相手の言葉を引用できない。根拠が自分の書いた文しかない
...
WHY が書かれないまま実装が進むと、レビューで「そもそもこれ必要?」となって全やり直しになる。スコープの議論が後出しになり「これも必要だった」が積み増される。

WHY が書かれているが合意されていない場合は、別の形で現れる。書いてあるためレビューは通り、下位セクションがその上に積まれる。後で WHY が棄却されると、下位の設計基準が反転する。たとえば推論した WHY の上では網羅的な一覧を作る設計になるはずだったものが、確定した WHY の上では網羅を目的から外す必要が出る。書き直すのは下位の記述だけでなく、下位を導いた基準そのものになる。手戻りコストは初期に WHY を詰めるコストの数倍である。
...
技術検証実装と、段階を分けて設計する場合の基盤（3-5）の境界はここにある。捨てられる成果物は土台にならず、土台になる成果物は捨てられない。次の段階が乗る土台として残すべき実行は、技術検証実装ではなく3-5の基盤phaseで扱う。
...
設計を尽くしたと言える単位は作業全体ではなく、実行可能な一段階である。技術検証実装は既に「書いたコードは捨ててよい。残るのは事実だけ」と定義している（3-4）が、捨てられない実行によってしか得られない事実はこの定義から外れる。基盤は作れば残り、次の段階が乗る土台になる。

段階を分けるのは、次の三条件をすべて満たすときである。

1. 実行して初めて確定する不確実性がある
2. その答えが、後続の設計判断の入力になる
3. 確定させるために作るものを捨てず、次の段階の土台として残す

三つ目が No なら技術検証実装で潰す（3-4）。捨てられる成果物は土台にならず、土台になる成果物は捨てられない。この区別が二つの手段を分ける。一つ目が No なら段階を分けない。既存の型が確立している作業ではこの条件がほとんど満たされず、現行どおり一度の design で完結する。

段階を分けた場合も、各段階の内側では既存の完了条件（未解消の設計判断がゼロ、分類保留がゼロ）を緩めない。「実装中に新しい判断が生まれない」という保証は各段階の内側で維持する。
...
**違反シグナル:**
- 実行しなければ確定しない論点を、机上の議論だけで決めさせようとしている
- 段階を分ける三条件を満たさないのに、段階を分けている
- 段階を分けたのに、各段階の TBD や分類保留の完了条件を緩めている
- `design.md` の対象範囲を明示せず、途中の design を最終形のように見せている

**帰結:**

実行して初めて確定する論点を机上の議論で決めさせ続けると、重箱の隅の議論に時間を消費し、基盤が立って初めて重要になる論点への着手が遅れる。逆に段階を無意味に増やすと、設計が細切れになり段階ごとの合意コストが積み上がる。

**問い:**
- 「この論点の答えは、実行して初めて確定するか?」
- 「その答えは、作るものそのものを変えるか?」
- 「いまの `design.md` は、どこまでを対象にしていると明示されているか?」
...
「議論の入力」ではなく「議論の結果」を記録する場所（3-6 思想に対応）。
...
2. 選んだ不確実性を、今回の段階のscopeへ含めるか判定する（3-5）。次の二問で判定する。
   - その論点の答えは、実行して初めて確定するか
   - その論点の答えは、作るものそのものを変えるか
   一つ目がYesで二つ目がNoなら、その段階のscopeへ含めない。`design.md`の非目標へ「実測後に設計する」と記し、この不確実性は今回のStep 3の対象から外す。両方Yesなら前に決める。実行しなければ確定しないため、下記の技術検証実装で潰す。一つ目がNoなら通常どおり解消手段を選ぶ。
   scopeへ含める場合は、解消手段を選ぶ前に、判断材料が枯れているかを [`making_and_choosing_options.md`](../../docs/think_standards/making_and_choosing_options.md) で確かめる。下の三手段は互いに排他的に書かれているため、`discussion`を選んだ時点で調査は終わったものとして扱われる。判定の中身は同 file が正本であり、ここへ写さない。
3. 不確実性の解消手段を選ぶ。
...
5. discussionでは、一つの論点でdecisionが返るたびに、そのdecisionだけを`design.md`の該当箇所へ直ちに反映する。調査・技術検証実装では、確定した事実だけを反映する。未決の提案や途中経過は書かず、複数decisionを最後まで溜めない。
6. decisionまたは事実から対象成果物の変更が生じるたびに、tasklist作成直前まで待たず、`design.md`付録のrouting stateへ分類する。
...
7. 一つのdecisionまたは事実を反映・分類するたびに、`design.md`全体から残る不確実性と完了条件を再評価する。残る場合はStep 3を繰り返し、なければStep 4へ進む。
...
- **新しい判断基準の検証**: 今回の設計で新しい判断基準または規則を設計した場合、起点となった具体caseだけで確認していないかを問う。起点以外の具体caseを二つ以上その基準へ当て、意図した場所へ落ちることを確認する
    - やってしまいがちな失敗: 起点caseで基準を作った直後、同じcaseで基準が満たされることを確認して終える。基準が起点case向けの特化ルールになっていることに気づけない
    - 判断基準: 起点以外の具体case二つ以上について、その基準を当てはめた結果を具体的に書けるか
- **確認範囲が主張範囲を覆っているか**: 検証可能な事実を`design.md`または会話で断定する前に、確認した範囲が主張する範囲を覆っているかを問う。「Xは3箇所にある」「この入力でYが出る」のような断定は、確認範囲が主張範囲より狭いと誤りになる
    - やってしまいがちな失敗: 一部だけ確認して全体を断定する。`grep`の探索先を一部のdirectoryへ絞る、出力を`head`で打ち切る、fileを読んだが該当分岐までは照合しない、といった形を取る。いずれも「確認した」感覚が残るぶん、未確認であることに気づけない
    - 判断基準: 主張が持つ量化子（すべて、N箇所、この入力で）に対し、確認の範囲が同じ量化子を満たしているか
    - 具体例（対象語の置換）: 用語変更・自称の言い換え等で`design.md`付録へ対象を列挙する場合、その列挙がrepository全体の網羅であることを`grep`の全出力で確認する。出力を`head`等で打ち切ったまま全体として扱わない。件数を確認するcommandが行数を数えるのか出現数を数えるのかを確認したうえで使う（`grep -c`は行数を数えるため、一行に複数出現するfileを過小に見積もる）。出力が多いfileほど見落としが起きやすい
    - 具体例（実行結果の断定）: 「この入力でこの出力が出る」をDoDまたは手順へ書く場合、対象の分岐を読み、その入力が実際にその分岐へ到達することを照合する。対象を読んだことと、該当分岐まで照合したことは別である
...
- [ ] ユーザー発言を脊髄反射で文書に反映した → 3-6
- [ ] 「どうお考えですか?」と先に問い、自分の見解を後出しにした → 3-6
- [ ] 既存 steering やドキュメントの文章をそのまま貼って提案にした → 3-6
- [ ] 推測で埋めた成果物を「レビューお願いします」と承認待ちにした → 3-6
- [ ] 指摘を受けた瞬間に合意なしでスキル・テンプレートを修正した → 3-6
...
- **転記禁止（3-6）** — steering や類似スキルの文章をそのまま貼らない。自分で咀嚼して提案する
...
- **合意なしに型を変えない（3-6）** — 「指摘されたから直す」を即実行しない。今のファイルで合意 → 型に反映
