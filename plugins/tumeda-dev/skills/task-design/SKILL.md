...
- **対象語の網羅確認**: 今回の設計が対象語を洗い出して置き換える設計（用語変更・自称の言い換え等）を含む場合、`design.md`付録の列挙がrepository全体の網羅であることを、`grep`の全出力で確認する。出力を`head`等で打ち切ったまま全体として扱わない。件数を確認するcommandが行数を数えるのか出現数を数えるのかを確認したうえで使う（`grep -c`は行数を数えるため、一行に複数出現するfileを過小に見積もる）
    - やってしまいがちな失敗: `grep`の出力を`head`で打ち切り、先頭だけを見て網羅したと判断する。出力が多いfileほど見落としが起きやすい
    - 判断基準: repository全体への`grep`を`head`を付けずに実行し、その全出力を確認した上で付録の列挙と突き合わせられるか
