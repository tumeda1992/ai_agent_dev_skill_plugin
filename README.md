...
- 共有するskill本文はこのpluginだけが保持する。利用先repositoryへ同じskill本文をコピーしない。
...
- hostごとのmodel差は`plugins/tumeda-dev/skills/runtime-model-profiles.md`の能力profileで吸収する。provider固有model名をskill手順が直接参照する基準にしない。
- このrepository自身も`<repository-root>/.agents/skills/tumeda-dev-plugin-context.md`のinstanceを持つ。pluginを開発する時もshared skillは同じcontract上で動くため、このrepositoryだけを例外にしない。

## 変更時の検証と前提

- このrepositoryの検証は `node scripts/verification/validate-plugin.mjs` の一つだけで行う。assertion（`requireText` / `requireExists` / `requireFrontmatter` / `requireAbsent` / `forbidText` / `requireOrderedText` 等）でskillとdocsの内容を検査する。test framework、lint、formatter、CI設定は他に存在しない。
